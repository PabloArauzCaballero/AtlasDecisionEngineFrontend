...
| `GET /v1/workers/identity-verification/reviews/{p}` | La cola de arbitraje trae cada caso ENTERO en el listado, así que la pantalla ya tiene lo que enseñaría el detalle y volver a pedirlo sólo añadiría una consulta. Se consumirá cuando el arbitraje tenga URL propia y se pueda compartir un caso por enlace.                 | portal      |
| `GET /v1/platform/catalog-manifest`                 | Manifiesto de siembra del catálogo: lo leen los guiones de despliegue para saber qué tablas se cargan. No describe nada que una persona decida desde el portal.                                                                                                              | plataforma  |
