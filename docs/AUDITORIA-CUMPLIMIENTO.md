...
| SELECT vs INPUT en formularios                  | ✅ Corregido                | `src/resources/catalog-fields.test.ts`                  | el barrido era manual, así que un campo nuevo podía volver a pedirse a mano  | sí  | —   | gate automático: todo campo cuyo concepto publica catálogo en `/v1/views/options` debe ofrecerlo (y todo campo, explicar qué poner)                             | `catalog-fields.test`  |
...
| Playwright de recorridos de error               | ✅ Corregido (2026‑07‑31)   | `e2e/error-tutorial.spec.ts`                            | sólo había pruebas unitarias del mapeo código→tutorial                       | sí  | —   | recorrido completo en el navegador: guardar falla (422) → aviso legible con «Ver tutorial guiado» → overlay abierto en su paso 1 → Escape lo cierra             | `yarn test:e2e`        |
...
- ~~**Barrido exhaustivo de SELECT en cada modal**~~: convertido en gate
  (`catalog-fields.test.ts`), que además obliga a que cada campo lleve su ayuda.
  `unitCode` queda a propósito como texto libre: el motor no publica catálogo de
  unidades y el vocabulario es abierto (cualquier moneda o magnitud).
- **Las 15 preguntas por vista** siguen parcialmente cubiertas (ver arriba).
- ~~**Playwright de error→tutorial**~~: hecho (`e2e/error-tutorial.spec.ts`). Al
  escribirlo salió a la luz que un guardado fallido en el editor se anunciaba
  **tres veces**: el aviso global que `QueryProvider` levanta para toda mutación
  fallida, un aviso local que era el único con el enlace al tutorial, y el
  diálogo bloqueante. Corregido: una vista puede declarar `meta.handled` en su
  mutación para que el aviso global se calle, y el diálogo del editor es ahora la
  única superficie — con la explicación del catálogo y el botón «Ver tutorial
  guiado» dentro. La prueba E2E exige que no haya ningún aviso a la vez que el
  diálogo, así que la duplicación no puede volver sin que falle.
